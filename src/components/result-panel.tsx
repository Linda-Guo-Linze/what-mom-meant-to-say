...
export function ResultPanel({ result, cards, sources, profile, onVoiceChange }: { result: SupportResult | null; cards: readonly KnowledgeCard[]; sources: readonly KnowledgeSource[]; profile?: LocalProfile; onVoiceChange: (voice: string, rate: number, pitch: number) => void }) {
  ...
  function play() {
    if (!result?.ttsAllowed || !result.simulatedWords || !("speechSynthesis" in window)) { setSpeechError("Audio is unavailable. The complete transcript remains visible."); return; }
    ...
    utterance.lang = utterance.voice?.lang ?? "en-US"; utterance.rate = rate; utterance.pitch = pitch;
    utterance.onend = () => setSpeaking(false);
    utterance.onerror = () => { setSpeaking(false); setSpeechError("This voice could not play. Try another installed English voice."); };
    setSpeechError(""); setSpeaking(true); window.speechSynthesis.speak(utterance); onVoiceChange(utterance.voice?.name ?? "", rate, pitch);
  }

...
import type { InterpretationInput, KnowledgeCard, KnowledgeSource, LocalProfile, SupportResult } from "../lib/schemas";
...
export function HomeWorkspace(props: {
  ...
  selectedCase: string; form: InterpretationInput; result: SupportResult | null; mode: "demo" | "live"; status: "idle" | "loading" | "error";
  notice: string; error: string; tourTarget?: string; onChooseCase: (item: DemoCase) => void; onChooseMode: (mode: "demo" | "live") => void;
  onGuide: () => void;
  onUpdate: (field: keyof InterpretationInput, value: string | string[]) => void; onSubmit: (event: FormEvent) => void; onVoiceChange: (voice: string, rate: number, pitch: number) => void;
}) {
  ...
  <ResultPanel key={`${result?.caseId ?? "custom"}-${result?.mode ?? "empty"}-${result?.riskLevel ?? "none"}`} result={result} cards={cards} sources={sources} profile={activeProfile} onVoiceChange={props.onVoiceChange} /></div></div>;
}
